...
def cargar_municipios():
    df_geo = pd.read_csv("municipios_colombia.csv")
    df_geo["Municipio"] = df_geo["Municipio"].str.upper().str.strip()
    df_geo["Departamento"] = df_geo["Departamento"].str.upper().str.strip()
    return df_geo

df_geo = cargar_municipios()
...
def coord_a_municipio(lat, lon):
    ...
    latitudes = df_geo["Latitud"].values
    longitudes = df_geo["Longitud"].values

    dlat = np.radians(latitudes - lat)
    dlon = np.radians(longitudes - lon)

    a = np.sin(dlat/2)**2 + np.cos(np.radians(lat)) * np.cos(np.radians(latitudes)) * np.sin(dlon/2)**2
    ...
    municipio = df_geo.iloc[idx]["Municipio"]
    depto = df_geo.iloc[idx]["Departamento"]

    resultado = f"{municipio}, {depto}"

    cache_lookup[key] = resultado
